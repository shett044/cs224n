...
def naiveSoftmaxLossAndGradient(
    ...
    # Word Vector are in row form
    U = outsideVectors
    u_o = U[outsideWordIdx]
    u_o = u_o.reshape(len(u_o), 1)

    v_c = centerWordVec
    v_c = v_c.reshape(len(v_c), 1)
    yw_i = np.exp(U.dot(v_c))
    sum_yw_i = np.sum(yw_i)
    loss = - u_o.T.dot(v_c).sum() + np.log(sum_yw_i)
    # Calculating the similarity of all vocabulary in v_c

    yw = yw_i / sum_yw_i

    # Creating  gradient of center_vector
    gradCenterVec = (- u_o + U.T.dot(yw)).ravel()

    # Creating gradient of outside vector
    gradOutsideVecs = yw.dot(v_c.T)
    gradOutsideVecs[outsideWordIdx] += - v_c.ravel()
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o_word in outsideWords:
        lossCurrent, gradC, gradO = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[o_word], outsideVectors, dataset)
