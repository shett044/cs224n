...
class CNN(nn.Module):
    def __init__(self, x_reshape, embd_word_len, k=5):
        ...
        print("CNN input")
        print("x_reshape, embd_word_len, k", x_reshape.size(), embd_word_len, k)
        self.k = k
        self.x_reshape = x_reshape
        self.embd_word_len = embd_word_len
        self.sentece_len, self.batch_size, self.embd_char_len, self.word_len = x_reshape.size()
    ...
    def forward(self):
        ...
        x_conv_out = x_conv_out.view(self.sentece_len, self.batch_size, self.embd_word_len)
        print("x_conv_out => ", x_conv_out.size())
